"""Safe selection and initialization of one Git-root Fab7 workspace."""
...
MAX_PARENT_WALK = 64
PROJECT_PATH = Path(".fab7/project.json")
...
def find_workspace(
    ...
    *,
    ...
    """Find and validate the explicit or nearest parent Fab7 workspace."""
    start = (cwd or Path.cwd()).resolve()
    ...
    for _ in range(MAX_PARENT_WALK):
        ...
        if state.is_symlink() or (state.exists() and not state.is_dir()):
            raise Fab7Error(
                "FAB7_PATH_INVALID",
                "Fab7 state path in workspace discovery is unsafe",
                {"path": str(state)},
            )
...
def _validate_project(root: Path) -> None:
    ...
    if not records.is_dir():
        raise Fab7Error(
            "FAB7_PROJECT_INVALID",
            "Fab7 records directory is missing or unsafe; rerun fab7 init",
            {"path": str(records)},
        )
...
def _require_safe_state_paths(state: Path, marker: Path) -> None:
    if state.is_symlink() or (state.exists() and not state.is_dir()):
        raise Fab7Error(
            "FAB7_PATH_INVALID",
            "Fab7 state directory must be a non-symlink directory",
            {"path": str(state)},
        )
    if marker.is_symlink() or (marker.exists() and not marker.is_file()):
        raise Fab7Error(
            "FAB7_PATH_INVALID",
            "Fab7 project marker must be a regular non-symlink file",
            {"path": str(marker)},
        )
...
def _require_safe_records_path(records: Path) -> None:
    if records.is_symlink() or (records.exists() and not records.is_dir()):
        raise Fab7Error(
            "FAB7_PATH_INVALID",
            "Fab7 records path must be a non-symlink directory",
            {"path": str(records)},
        )
...
def _read_marker(path: Path) -> dict[str, int]:
    try:
        ...
        raise Fab7Error(
            "FAB7_PROJECT_INCOMPATIBLE",
            "Fab7 project marker is incompatible; archive or remove only the marker and rerun fab7 init",
            {"path": str(path)},
        ) from exc
...
def _not_initialized(path: Path) -> Fab7Error:
    return Fab7Error(
        "FAB7_PROJECT_NOT_INITIALIZED",
        "No Fab7 project was found; run fab7 init or pass --workspace",
        {"path": str(path)},
    )
