...
def test_standard_package_metadata_is_lean_and_aligned() -> None:
    ...
    locked_project = next(row for row in lock["package"] if row["name"] == "fab7-cli")

    assert __version__ == project["project"]["version"] == locked_project["version"] == "0.1.0"
    assert project["project"]["requires-python"] == ">=3.11"
    assert project["project"]["dependencies"] == []
    assert project["project"]["scripts"] == {"fab7": "fab7.cli:main"}
    assert project["tool"]["fab7"]["recommended-uv-version"] == "0.11.32"
...
def test_wheel_contains_only_the_proof_runtime(tmp_path: Path) -> None:
    ...
    wheel = next(tmp_path.glob("fab7_cli-0.1.0-*.whl"))
    with zipfile.ZipFile(wheel) as archive:
        names = set(archive.namelist())

    assert {f"fab7/{name}" for name in (
...
def test_import_boundary_loads_only_proof_core_modules() -> None:
    process = subprocess.run(
        [
            ...
            "-c",
            "import sys, fab7.cli; "
            "print('\\n'.join(sorted(name for name in sys.modules if name.startswith('fab7.'))))",
        ],
        cwd=ROOT,
        text=True,
        stdout=subprocess.PIPE,
...
def test_static_host_plugins_are_short_and_runtime_free() -> None:
    codex = ROOT / "plugins/codex/fab7"
    claude = ROOT / "plugins/claude/fab7"
    ...
    assert codex_manifest["name"] == claude_manifest["name"] == "fab7"
    ...
    for path in [*codex.rglob("*"), *claude.rglob("*")]:
        if path.is_file():
            ...
            for retired in ("pyinstaller", "fab7 home", "native executable", "extension catalog"):
