...
from cuff.gate import check
from cuff.ledger import append, create_claim, init, record_path, seal, verify
from cuff.subject import declared_subject, filesystem_subject
...
def test_gate_has_one_closed_projection_and_requires_latest_claim(repo: Path) -> None:
    ...
    assert _codes(empty) == ["CUFF_CLAIM_MISSING"]
...
def test_explicit_nonancestor_base_fails_readiness(repo: Path) -> None:
    ...
    assert "CUFF_GIT_ANCESTRY" in _codes(result)
    assert not result["ok"]
